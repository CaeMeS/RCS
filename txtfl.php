...
.buttonsha:hover {
    ...
    <body bgcolor="grey" text="black" vlink="#FF9933" link="#FF0033">
        ...
        <div style="position: absolute; top: 260; left: 400; width: 1000px; text-align:left;">
            ...
            <button class="buttonshadow buttonsha" onclick="location.href='http://localhost/RCS/txtnews.php'" type="button">News</button> 
            <button class="buttonshall buttonsha" onclick="location.href='http://localhost/RCS/maintexts.php'" type="button">Show All</button> <br> 
            <p style="font-size: 20px; color: #FF9933;"> Category: FIRST LOOK </p> <br>
           <?php 
            if($_SERVER["REQUEST_METHOD"] == "POST")
                {
                    $szukaj = mysqli_real_escape_string($link, $_POST['szukaj']);
                    $txt ="select id_tekstu, temat, kategoria from TEKST where temat like '%$szukaj%' and kategoria = 'FIRSTLOOK' order by data_publikacji desc";
                    ?>
                ...
                {
                    $txt ="select id_tekstu, temat, kategoria from TEKST where kategoria = 'FIRSTLOOK' order by data_publikacji desc";
                }
