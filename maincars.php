...
.button:hover span:after {
  ...
  right: 0;
    ...
    <body bgcolor="grey" text="black" vlink="#FF9933" link="#FF0033">
        ...
        <div style="position: absolute; top: 320; left: 400; width: 1000px; text-align:left;">
           <?php 
                if($_SERVER["REQUEST_METHOD"] == "POST")
                {
                    $szukaj = mysqli_real_escape_string($link, $_POST['szukaj']);
                    $cars ="select id_samochodu, rok, silnik, marka, model, foto from AUTO where model like '%$szukaj%' or marka like '%$szukaj%' or silnik like '%$szukaj%' or rok like '%$szukaj%' order by data_dodania desc";
                    ?>
                    <p style="font-size: 20px; color: black;"> Search for: <i> <?php echo htmlspecialchars("$szukaj "); ?> </i> 
                    <button style="color: #FF0033" onclick="location.href='http://localhost/RCS/maincars.php'" type="button">X</button> </p>  <br> <br>
                    <?php
                }
                else
                {
                    $cars ="select id_samochodu, rok, silnik, marka, model, foto from AUTO order by data_dodania desc";
                }
                    $result = mysqli_query($link, $cars);   
                    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
                    {                    
	                       $idcar=$row['id_samochodu'];
	                       $rok=$row['rok'];
                           $silnik=$row['silnik'];   
                           $marka=$row['marka']; 
                           $model=$row['model']; 
                    ...
                    <div style="font-size: 25pt;">
                        <a style="text-decoration: none" href = "car.php?id=<?php echo $idcar; ?>" > <img src="cars/<?php echo $foto; ?>" alt="Car Photo" style="width:256px;height:144px;"> </a>
                        <a style="text-decoration: none" href = "car.php?id=<?php echo $idcar; ?>" > <?php echo htmlspecialchars($marka); echo htmlspecialchars(" $model"); echo htmlspecialchars(" $rok"); echo htmlspecialchars(" $silnik"); ?> </a> <br>
                    </div>      
                    <?php
                        }
                    ?>
